...
// ProxifyEncoder is the structure containing the writer used when encoding Envelopes
type ProxifyEncoder struct {
	writer io.Writer
}

// NewEncoder encode Ligolo-ng packets
func NewEncoder(writer io.Writer) ProxifyEncoder {
	return ProxifyEncoder{writer: writer}
}

// Encode encode an Envelope packet and write the result into the writer
func (e *ProxifyEncoder) Encode(envelope Envelope) error {
